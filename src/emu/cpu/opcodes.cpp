...
void CPU::ctc2()
{
	gte.write_reg(i.r_type.rd + 32, regs[i.r_type.rt]);
	if (can_disassemble) printf("ctc2 r%d, %s\n", i.r_type.rd + 32, Reg(i.r_type.rt));
}
...
void CPU::op_lb()
{
    ...
    if (!isCacheIsolated())
    {
        uint32_t value = (int8_t)bus->read<uint8_t>(vaddr);
		load(rt, value);
    }
...
void CPU::op_lh()
{
    ...
    if (!isCacheIsolated())
    {
		...
		uint32_t data = (int16_t)bus->read<uint16_t>(vaddr);
		load(rt, data);
    }
...
void CPU::op_lwl()
{
	...
	uint32_t mem = bus->read<uint32_t>(addr & ~0x3);
...
void CPU::op_lw()
{
    ...
    if (!isCacheIsolated())
    {
		...
		uint32_t data = bus->read<uint32_t>(vaddr);
		load(rt, data);
    }
...
void CPU::op_lbu()
{
    ...
    if (!isCacheIsolated())
    {
        uint32_t value = bus->read<uint8_t>(vaddr);
		load(rt, value);
    }
...
void CPU::op_lhu()
{
    ...
    if (!isCacheIsolated())
    {
        ...
        uint32_t value = bus->read<uint16_t>(vaddr);
		load(rt, value);
    }
...
void CPU::op_sb()
{
    ...
    if (!isCacheIsolated())
        bus->write<uint8_t>(addr, regs[rt]);
}
...
void CPU::op_sh()
{
    ...
    if (!isCacheIsolated())
        bus->write<uint16_t>(addr, regs[rt]);
}
...
void CPU::op_swl()
{
	...
	bus->write<uint32_t>(addr & ~0x3, (regs[source] >> SWL_SHIFT[shift]) | (mem & SWL_MASK[shift]));
}
...
void CPU::op_swr()
{
	...
	bus->write<uint32_t>(addr & ~0x3, (regs[source] << SWR_SHIFT[shift]) | (mem & SWR_MASK[shift]));
}
